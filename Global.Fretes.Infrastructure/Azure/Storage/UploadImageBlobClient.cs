...
namespace Global.Fretes.Infrastructure.Azure.Storage;

public sealed class UploadImageBlobClient : IUploadImageBlobClient
{
    public async Task<bool> DeleteImageAsync(string nomeFoto)
    {
        try
        {
            var blobCliente = new BlobClient(ConfigKeyAzureContainer.Key, ConfigKeyAzureContainer.Container, nomeFoto);
